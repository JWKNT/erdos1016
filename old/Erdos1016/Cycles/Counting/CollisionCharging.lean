...
theorem nonsimpleCyclicRuns_card_le_split_bound (ell s D : ℕ)
  ...
  exact bad_runs_card_le_of_positioned_split_code G ell s hmin hmax
...
theorem normalizedCollisionMass_le_trace (L s D : ℕ)
  ...
  intro ell hell hsle
  exact nonsimpleCyclicRuns_card_le_split_bound G ell s D
    (by omega) hmin hmax hg hshort hs
